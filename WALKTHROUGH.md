...
> construction and supply chain. This is not new capability. It is existing
> capability pointed at defence."
...
**Then scroll to "Design and assurance, with the client in control."**
> "We are provider agnostic on purpose. It keeps the platform decision open until the
> requirement is written, which is the right order and usually the cheaper one. And on the
> bigger programmes we work in joint venture and name the partner in the bid, so you get one
